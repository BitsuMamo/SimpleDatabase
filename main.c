...
/*
 * Takes a Node
 * Returns the number of cells found in the node
...
/*
 * Takes a Node and a cell numbber
 * Gets the memory of the leaf node cell
...
/*
 * Takes a Node and cell number
 * Gets the key pointer for a node cell
 * Returns uint32_t pointer to the key
 * */
...
/*
 * Takes a Node and cell number
 * Gets the memory location of the leaf value
...
/*
 * Takes a Node
 * Sets the number of cell for a node to 0
 * */
...
/*
 * Takes a Cursor, Key and Value
 * Gets the node from the cursor. Checks if the page can input new data.
 * If our cursor cell number is less that the available space shift memory back
 * one and add data to the cursor cell number.
 * */
...
void leaf_node_split_and_insert(Cursor *cursor, uint32_t key, Row *value) {
   ...
   * Insert new value in one of the two nodes.
   * Update the parent or create new parent.
   */
  ...
  *leaf_node_next_leaf(old_node) = 0;
...
uint32_t *internal_node_child(void *node, uint32_t child_num) {
  ...
  if (child_num > num_keys) {
    printf("Tried to accesss child num %d > num keys %d\n", child_num,
           num_keys);
    exit(EXIT_FAILURE);
...
void create_new_root(Table *table, uint32_t right_child_page_num) {
   ...
   * Old root copid to new page and becomes the left child.
   * Address of right child passed in.
   * Re-initialized root page to containt he new root node.
   * New root node point to two children.
   */
 ...
 * Copys the data from the row to the page memory found at the destincation
 * memory. Basic operation is writing to file
 * */
...
void close_input_buffer(InputBuffer *input_buffer) {
  // Need to free space for both the buffer allocated throught the get line
  // method and the struct which we allocate when we call new_input_buffer
  // method
...
/*
 * Take filename
 * Using the file name it opens a connection to a file. Stores the connection
 * and file lenght inside the Pager struct. Initializeds all cached pages foudn
 * in Pager to null. Returns a Pager.
 * */
...
/*
 * Until we start recycling free pages newpages go to the end of the file.
 */
uint32_t get_unused_page_num(Pager *pager) { return pager->num_pages; }

/*
 * Takes pager and page num
 * Checks if the page exists and grabs it. If not out of memory creates a new
 * page. Retuns the wanted page.
 * */
...
void read_input(InputBuffer *input_buffer) {
  // While starting since buffer is Null getline will allocate the memory
  // needed.
  ssize_t bytes_read =
 ...
 * Checks for different SQL statements and assigns the statement->type to the
 * correct enum value. Currently we check and do the this for the user table
 * hard coded. The select print all data. Returns an enum value with an error or
 * success.
 * */
 ...
 * Based on the Statement->type it will run the execute_insert or execute_select
 * functions. Returns enum value of success or error
 * */
...
/*
 * Main function with main loop and proper handling of returns from other
 * functions
 * */
